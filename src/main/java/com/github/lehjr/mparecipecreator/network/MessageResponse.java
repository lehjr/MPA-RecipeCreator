    ...
    public static class Handler implements IMessageHandler<MessageResponse, IMessage> {
        ...
        public IMessage onMessage(MessageResponse message, MessageContext ctx) {
            if (message.status == Status.OK) {
                ...
                MPALibLogger.logger.info("Something went wrong: {}. Message: {}", message.status.name(), message.message);
                if (Minecraft.getMinecraft().currentScreen instanceof MPARCGui) {
                    // Fixme: IntelliJ are you drunk?
                    //noinspection ConstantConditions
                    System.out.println(message.message);
//                    ((MPARCGui) Minecraft.getMinecraft().currentScreen).showMessage(message.message);
                }
            }
            return null;
        }
